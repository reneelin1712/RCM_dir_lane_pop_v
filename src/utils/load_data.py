...
def load_link_feature(edge_path):
    # Read the edge file including the necessary columns
    edge_df = pd.read_csv(edge_path, usecols=['highway', 'length', 'lanes', 'n_id', 'ratio'], dtype={'highway': str, 'lanes': str})
    ...
    # Concatenate length, lanes, highway type, and ratio into the edge feature array
    edge_feature = np.concatenate([
        np.expand_dims(edge_df['length'].values, 1),  # Add length
        np.expand_dims(edge_df['lanes'].values, 1),   # Add lanes
