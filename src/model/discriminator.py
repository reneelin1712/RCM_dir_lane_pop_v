...
class DiscriminatorAIRLCNN(nn.Module):
    def __init__(self, action_num, gamma, policy_mask, action_state, path_feature, link_feature, rs_input_dim,
        ...
        self.conv1 = nn.Conv2d(rs_input_dim + 1, 20, 3, padding=1)  # Increased input dimension by 1
        self.pool = nn.MaxPool2d(2, 1)
        self.conv2 = nn.Conv2d(20, 30, 2)
        self.fc1 = nn.Linear(30, 120)  # Removed action_num from input dimension
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 1)

        self.h_fc1 = nn.Linear(hs_input_dim + 1, 120)  # Increased input dimension by 1
        self.h_fc2 = nn.Linear(120, 84)
        self.h_fc3 = nn.Linear(84, 1)
    ...
    def process_neigh_features(self, state, des, time_step):
        ...
        neigh_mask_feature = self.policy_mask_pad[state].unsqueeze(-1)
        ...
        for i in range(state.size(0)):
            ...
            for neighbor in state_neighbor[i]:
                speed = self.speed_data.get((neighbor.item(), time_step[i].item()), 0)
    ...
    def process_state_features(self, state, des, time_step):
        ...
        speed_features = []
        for i in range(state.size(0)):
            speed = self.speed_data.get((state[i].item(), time_step[i].item()), 0)
    ...
    def f(self, state, des, next_state, time_step):
        """rs"""
    ...
    def forward(self, states, des, log_pis, next_states, time_steps):
        # Discriminator's output is sigmoid(f - log_pi).
        return self.f(states, des, next_states, time_steps) - log_pis

    def calculate_reward(self, states, des, log_pis, next_states, time_steps):
        with torch.no_grad():
            logits = self.forward(states, des, log_pis, next_states, time_steps)
            return -F.logsigmoid(-logits)

    def get_input_features(self, state, des, next_state):
        state_neighbor = self.action_state_pad[state]
        neigh_path_feature = self.path_feature[state_neighbor, des.unsqueeze(1).repeat(1, self.action_num + 1), :]
        neigh_edge_feature = self.link_feature[state_neighbor, :]
    ...
    def forward_with_actual_features(self, neigh_path_feature, neigh_edge_feature, path_feature, edge_feature, log_prob, next_path_feature, next_edge_feature, time_step):
        # Calculate the neigh_mask_feature
        neigh_mask_feature = self.policy_mask_pad[self.cur_state].unsqueeze(-1)

        # Get speed features for the current state
        speed_features = []
        for i in range(self.cur_state.size(0)):
            speed = self.speed_data.get((self.cur_state[i].item(), time_step[i].item()), 0)
            speed_features.append(speed)
        ...
        speed_feature = speed_feature.view(1, 1, -1)
        path_feature = path_feature.unsqueeze(0) if path_feature.dim() == 1 else path_feature
        edge_feature = edge_feature.unsqueeze(0) if edge_feature.dim() == 1 else edge_feature
        ...
        next_speed_features = []
        for i in range(self.cur_state.size(0)):
            next_speed = self.speed_data.get((self.cur_state[i].item(), time_step[i].item()), 0)
        ...
        next_speed_feature = next_speed_feature.view(1, 1, -1)
        next_path_feature = next_path_feature.unsqueeze(0) if next_path_feature.dim() == 1 else next_path_feature
        next_edge_feature = next_edge_feature.unsqueeze(0) if next_edge_feature.dim() == 1 else next_edge_feature
